...
class VoterDatabaseTestSuite {
  ...
  void voterTest() {
    ...
    Voter completeVoter = new Voter(createDate(20010725), 'D', "Connor"); // Voter(Date, char,
                                                                          // String)
    ...
    if (!(completeVoter.getParty() == 'D')) {
      fail("The party that was returned did not match the party that the Voter was "
          + "created with");
    }

    if (!(completeVoter.getBirthday().equals(createDate(20010725)))) {
      fail("The birthday that was returned did not match the birthday that the Voter was "
          + "created with");
    }
  ...
  void getVoterTest() {
    ...
    if (!(voterDatabase.getVoter(20010725).getBirthday().equals(a.getBirthday()))
      ...
      if (voterDatabase.getVoter(19990826) != null) {
  ...
  public Date createDate(int birthDate) {
    try {
      ...
      String formattedDate = year + "-" + month + "-" + date; // Creates a formated date.
